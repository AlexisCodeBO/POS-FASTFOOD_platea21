...
<?php include('parciales/foot_cabecera.php') ?>	
		...
		include('Sitio3.php');
		if (isset($_GET['id'])){
			$arr=Sitio3::ListarSitio3($_GET['id']);
		} 
		$sw = isset($arr);
		
		?>
		<div class="container">
		<form action="" method="POST" role="form" id="fsitio3">
			<?php if ($sw) { ?>
				<legend>Modificacion Producto</legend>
			<?php }
			else {
			 ?>
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="nombre" name="nombre" value="<?=$sw?$arr[0]->getNombre():''?>" placeholder="Nombre Producto">
			</div>
			<div class="form-group">
		...
		<script type="text/javascript">
			var f = document.getElementById('fsitio3');
			f.onsubmit=insertar;
			function insertar(e) {
				e.preventDefault();
				...
				if (btn == 'Modificar') {
					xht.open('POST','modificarSitio3.php');
				}
				else{
					xht.open('POST','insercionSitio3.php');
				}
